//
//  MTLockNode.m
//  手势解锁
//
//  Created by 王奕聪 on 2018/3/17.
//  Copyright © 2018年 王奕聪. All rights reserved.
//


#import "MTLockNode.h"
#import "MTLockViewConfig.h"

@interface MTLockNode()
{
    MTLockViewConfig* _model;
}
/**
 *  外环颜色
 */
@property (nonatomic, strong) UIColor *outCircleColor;

/**
 *  实心圆颜色
 */
@property (nonatomic, strong) UIColor *inCircleColor;

/**
 *  三角形颜色
 */
@property (nonatomic, strong) UIColor *trangleColor;

@end

@implementation MTLockNode

-(instancetype)initWithFrame:(CGRect)frame
{
    if([super initWithFrame:frame])
    {
        [self setupDefault];
    }
    
    return self;
}

- (id)initWithCoder:(NSCoder *)aDecoder
{
    if (self = [super initWithCoder:aDecoder]) {
        [self setupDefault];
    }
    return self;
}

-(void)setModel:(MTLockViewConfig *)model
{
    _model = model;
    
    [self setNeedsDisplay];
}

-(MTLockViewConfig *)model
{
    if(!_model)
    {
        _model = [MTLockViewConfig new];
    }
    
    return _model;
}

-(void)setupDefault
{
    self.backgroundColor = [UIColor clearColor];
    self.userInteractionEnabled = NO;
    
//    [self setImage:[UIImage imageNamed:@"gesture_node_normal"] forState:UIControlStateNormal];
//    //        //设置按钮选中的状态
//    [self setImage:[UIImage imageNamed:@"gesture_node_selected"] forState:UIControlStateSelected];
}

- (void)drawRect:(CGRect)rect {

    CGContextRef ctx = UIGraphicsGetCurrentContext();
    
    CGFloat radio = self.model.circleRadio;
    CGRect circleRect = CGRectMake(self.model.edgeWidth, self.model.edgeWidth, rect.size.width - 2 * self.model.edgeWidth, rect.size.height - 2 * self.model.edgeWidth);
    
    
    
    // 上下文旋转
    [self transFormCtx:ctx rect:rect];
    
    // 画圆环
    [self drawEmptyCircleWithContext:ctx rect:circleRect color:self.outCircleColor];
    
    // 画实心圆
    [self drawSolidCircleWithContext:ctx rect:rect radio:radio color:self.inCircleColor];
    
    if (self.model.arrow) {
        
        // 画三角形箭头
        [self drawTrangleWithContext:ctx topPoint:CGPointMake(rect.size.width/2, 10) length:self.model.trangleLength color:self.trangleColor];
    }
}

#pragma mark - 画外圆环
/**
 *  画外圆环
 *
 *  @param ctx   图形上下文
 *  @param rect  绘画范围
 *  @param color 绘制颜色
 */
- (void)drawEmptyCircleWithContext:(CGContextRef)ctx rect:(CGRect)rect color:(UIColor *)color
{
    CGMutablePathRef circlePath = CGPathCreateMutable();
    CGPathAddEllipseInRect(circlePath, NULL, rect);
    CGContextAddPath(ctx, circlePath);
    [color set];
    CGContextSetLineWidth(ctx, self.model.edgeWidth);
    
    if(self.model.isEdgeFill)
        CGContextFillPath(ctx);
    else
        CGContextStrokePath(ctx);
    
    CGPathRelease(circlePath);
}

#pragma mark - 画实心圆
/**
 *  画实心圆
 *
 *  @param ctx   图形上下文
 *  @param rect  绘制范围
 *  @param radio 占大圆比例
 *  @param color 绘制颜色
 */
- (void)drawSolidCircleWithContext:(CGContextRef)ctx rect:(CGRect)rect radio:(CGFloat)radio color:(UIColor *)color
{
    CGMutablePathRef circlePath = CGPathCreateMutable();
    CGPathAddEllipseInRect(circlePath, NULL, CGRectMake(rect.size.width/2 * (1 - radio) + self.model.edgeWidth, rect.size.height/2 * (1 - radio) + self.model.edgeWidth, rect.size.width * radio - self.model.edgeWidth * 2, rect.size.height * radio - self.model.edgeWidth * 2));
    [color set];
    CGContextAddPath(ctx, circlePath);
    CGContextFillPath(ctx);
    CGPathRelease(circlePath);

}

#pragma mark - 画三角形
/**
 *  画三角形
 *
 *  @param ctx    图形上下文
 *  @param point  顶点
 *  @param length 边长
 *  @param color  绘制颜色
 */
- (void)drawTrangleWithContext:(CGContextRef)ctx topPoint:(CGPoint)point length:(CGFloat)length color:(UIColor *)color
{
    CGMutablePathRef trianglePathM = CGPathCreateMutable();
    CGPathMoveToPoint(trianglePathM, NULL, point.x, point.y);
    CGPathAddLineToPoint(trianglePathM, NULL, point.x - length/2, point.y + length/2);
    CGPathAddLineToPoint(trianglePathM, NULL, point.x + length/2, point.y + length/2);
    CGContextAddPath(ctx, trianglePathM);
    [color set];
    CGContextFillPath(ctx);
    CGPathRelease(trianglePathM);
}

/*
 *  上下文旋转
 */
-(void)transFormCtx:(CGContextRef)ctx rect:(CGRect)rect{
    //    if(self.angle == 0) return;
    CGFloat translateXY = rect.size.width * .5f;
    //平移
    CGContextTranslateCTM(ctx, translateXY, translateXY);
    CGContextRotateCTM(ctx, self.angle);
    //再平移回来
    CGContextTranslateCTM(ctx, -translateXY, -translateXY);
}




- (UIColor *)outCircleColor
{
    return self.model.outCircleColors[self.circleState];
}

- (UIColor *)inCircleColor
{
    return self.model.inCircleColors[self.circleState];
}

- (UIColor *)trangleColor
{
    return self.model.trangleColors[self.circleState];
}

- (void)setAngle:(CGFloat)angle
{
    _angle = angle;
    
    [self setNeedsDisplay];
}

-(void)setCircleState:(MTLockNodeState)circleState
{
    _circleState = circleState;
    
    [self setNeedsDisplay];
}

@end

