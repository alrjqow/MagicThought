...
@interface MTSlideHeader ()

/*------------其他设置------------*/
/**
 *  MLMSegmentHeadStyle
 */
@property (nonatomic, assign) MTSegmentHeadStyle headStyle;

/**
 *  MLMSegmentHeadStyle
 */
@property (nonatomic, assign) MTSegmentLayoutStyle layoutStyle;

@end


@implementation MTSlideHeader
{
...
- (instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titles {
    return [self initWithFrame:frame titles:titles headStyle:MTSegmentHeadStyleDefault];
}

- (instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titles headStyle:(MTSegmentHeadStyle)style {
    return [self initWithFrame:frame titles:titles headStyle:style layoutStyle:MTSegmentLayoutDefault];
}

- (instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titles headStyle:(MTSegmentHeadStyle)style layoutStyle:(MTSegmentLayoutStyle)layout {
...
- (void)defaultAndCreateView {
    ...
    if (_equalSize) {
        ...
        if (titlesScroll) {
            titlesScroll.width = mt_ScreenW();
        }
        
        if (slideScroll) {
            slideScroll.width = mt_ScreenW();
        }
    }
    
    //判断总宽度
    if (sum_width > SCROLL_WIDTH && _layoutStyle== MTSegmentLayoutCenter) {
        _layoutStyle = MTSegmentLayoutLeft;
    }
...
- (void)titlesWidth {
    ...
    for (NSString *title in titlesArray) {
        if (_layoutStyle != MTSegmentLayoutDefault) {
...
- (CGFloat)titleWidth:(NSString *)title {
    if(self.onlyUseSingleW) return _singleW_Add;
...
- (void)createView {
    ...
    _fontScale = _headStyle==MTSegmentHeadStyleSlide?1:_fontScale;
...
- (void)scrollViewSubviews:(UIScrollView*)scroll {
    ...
    CGFloat start_x = 0;
    if(self.marginLeft) start_x = self.marginLeft;
    else if (_layoutStyle == MTSegmentLayoutCenter) {
        //计算布局的起点
    ...
    if (titles && _headStyle != MTSegmentHeadStyleSlide) {
...
- (void)selectedHeadTitles:(UIButton *)button {
    ...
    [UIView animateWithDuration:animation_time animations:^{
        if (_headStyle != MTSegmentHeadStyleSlide) {
    ...
    isSelected = YES;
    if ([self.delegate respondsToSelector:@selector(didSelectedWithHeader:Index:)]) {
        [self.delegate didSelectedWithHeader:self Index:selectIndex];
    }
...
- (void)changePointScale:(CGFloat)scale {
    ...
    switch (_headStyle) {
        case MTSegmentHeadStyleDefault:
        case MTSegmentHeadStyleLine:
        case MTSegmentHeadStyleArrow:
        {
            if (lineView) {
...
- (UIView *)getScrollLineView {
    if (_headStyle == MTSegmentHeadStyleLine) {
        return lineView;
    } else {
        return nil;
    }
